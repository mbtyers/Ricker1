...
server <- function(input, output) {
    ...
    simulateSR <- function(lnalpha, beta, cvS, hrange, sig, N, phi) {
        ...
        S <- Seq*runif(1, 1-hrange[2], 1-hrange[1])
        
        # initial value for observed S
        Sobs <- S*rlnorm(1, sdlog=siglog)
        
        # calculate true initial R from true initial S
        fits <- lnalpha - beta*S  # true SR 
        resids <- rnorm(1, 0, sig)
        R <- exp(fits+resids)*S
        ...
        # ----- recursive portion ----- #
        for(i in 2:N) {
            # this year's escapement is last year's recruits minus last year's harvest
            S[i] <- R[i-1] - H[i-1]
            
            # this year's escapement is observed with measurement error
            Sobs[i] <- S[i]*rlnorm(1, sdlog=siglog)
            
            # last year's recruits are observed as this year's observed escapement plus harvest 
            Robs[i-1] <- Sobs[i] + H[i-1]
            
            # calculate true R from true S
            fits[i] <- lnalpha - beta*S[i]
            resids[i] <- phi*resids[i-1] + rnorm(1, 0, sig)
            R[i] <- exp(fits[i]+resids[i])*S[i]
            
            # this year's harvest from this year's R
            H[i] <- R[i]*runif(1, hrange[1], hrange[2])
        }
        
        # observe final year's recruitment in the same awkward way
        Robs[N] <- (R[N-1] - H[N-1])*rlnorm(1, sdlog=siglog) + H[N-1]
    ...
    output$ResidPlot <- renderPlot({
        if(input$sim) {
            ...
            abline(h=0, lty=3)
            text(1, 1.2*plotmax, paste("D-W stat:", round(durbinWatsonTest(Rfit$resids),2)), pos=4)
